...
from ..script_supporting_functions import GPUUtilizationLogger, log_model_complexity, build_backbone
import math
import sqlite3
...
def main(config_file):
    ...
    callbacks = [
        ...
        GPUUtilizationLogger(
            log_every_n_steps=config["logs"]["steps"],
            device_index=config["training"]["fit"].get("devices", [0])[0] if isinstance(config["training"]["fit"].get("devices", [0]), list) else 0,
        ),
    ]
