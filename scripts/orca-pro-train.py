...
def main(config_file):
    ...
    # Do the selection — filter mc_truth to only the selected events so we
    # never load the full table (potentially millions of rows) into RAM.
    ...
    chunk_size = 999  # SQLite's default SQLITE_MAX_VARIABLE_NUMBER limit
    rows = []
    for i in range(0, len(selection_list), chunk_size):
        chunk = selection_list[i:i + chunk_size]
        placeholders = ",".join("?" * len(chunk))
        c_truth.execute(
            f"SELECT event_no, initial_state_type, interaction FROM mc_truth "
            f"WHERE event_no IN ({placeholders})",
            chunk,
        )
        rows.extend(c_truth.fetchall())
    truth_conn.close()
    mc_truth_info = np.array(rows)
    ...
    logger.info(f'Number of training events: {len(training_dataloader)*config["dataloader"]["batch_size"]}')
    logger.info(f'Number of validation events: {len(validation_dataloader)*config["dataloader"]["batch_size"]}')
